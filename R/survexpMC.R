...
#' ##
#' data(headnneck)
#' survexpMC(8,12,4,7,2,10,headnneck)
#' ##
#' }
#' @seealso survweibMC
...
survexpMC<-function(m1,n1,m2,n2,chains,iter,data){
  ...
  modelfe1 <- file.path(tempdir(), "exp1.txt")
  write.model(exp1, modelfe1)
  ...
  v100.sim <- bugs(a, inits, model.file = modelfe1,
                   parameters.to.save = params1, n.chains =chains,
                   n.iter =iter,n.burnin =(iter/2))
  survexpMCout <- list()
  result <- v100.sim$summary
  survexpMCout[["Posterior estimates"]]<-result
  survexpMCout[["DIC"]]<-v100.sim$DIC
  return(survexpMCout)
}
utils::globalVariables(c("N","beta1","U","Y","yp1","b","beta2","r1","r2","sur","tauz","inverse","sigmab1","sigmab2","cor"))
